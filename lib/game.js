(function() {
  ...
  Game.prototype.draw = function (ctx) {
    ...
    for (var i in this.allObj) {
      if (this.allObj[i] instanceof Asteroids.ExplosionShard) {
        // debugger
      }
    ...
    for (i = 0; i < allObjects.length - 1; i++) {
      for (j = i + 1; j < allObjects.length; j++) {
        if (allObjects[i].isCollidedWith(allObjects[j])) {
          ...
          if (!(firstObject instanceof Asteroids.ExplosionShard || secondObject instanceof Asteroids.ExplosionShard)) {
