(function() {
  ...
  Game.prototype.addStars = function() {
    for (i = 0; i < this.NUM_STARS; i++) {
      this.stars.push(new Asteroids.Star({game: this, pos: this.randomPosition()}))
    };
  };

  Game.prototype.allObjects = function () {
	   this.allObj = this.asteroids.concat(this.bullets, this.ship, this.explodingAsteroids, this.explosionShards, this.stars);
     return this.allObj;
  };
    ...
    for (i = 0; i < allObjects.length - 1; i++) {
      for (j = i + 1; j < allObjects.length; j++) {
        if (allObjects[i].isCollidedWith(allObjects[j])) {
          ...
          if (!(firstObject instanceof Asteroids.ExplosionShard || secondObject instanceof Asteroids.ExplosionShard || firstObject instanceof Asteroids.Star || secondObject instanceof Asteroids.Star || firstObject instanceof Asteroids.explodingAsteroid || secondObject instanceof Asteroids.explodingAsteroid)) {
  ...
  Game.prototype.remove = function(targetObject) {
    if (targetObject instanceof Asteroids.Bullet) {
      ...
      var index = this.explodingAsteroids.indexOf(targetObject);
      this.explodingAsteroids.splice(index, 1);
    } else if (targetObject instanceof Asteroids.ExplosionShard) {
      var index = this.explosionShards.indexOf(targetObject);
